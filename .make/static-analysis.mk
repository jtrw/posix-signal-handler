...
lint: ## checks syntax of PHP files
	docker-compose run --rm --no-deps $(PHP_FPM) sh -lc './vendor/bin/parallel-lint ./ --exclude vendor --exclude bin/.phpunit'
	docker-compose run --rm --no-deps $(PHP_FPM) sh -lc './bin/console lint:yaml config'

.PHONY: layer
layer: ## check issues with layers (deptrac tool)
	docker-compose run --rm --no-deps $(PHP_FPM) sh -lc './vendor/bin/deptrac analyze --formatter-graphviz=0'

.PHONY: coding-standards
coding-standards: ## run check and validate code standards tests
	docker-compose run --rm --no-deps $(PHP_FPM) sh -lc './vendor/bin/ecs check src tests'
	docker-compose run --rm --no-deps $(PHP_FPM) sh -lc './vendor/bin/phpmd src/ text phpmd.xml'

.PHONY: coding-standards-fixer
coding-standards-fixer: ## run code standards fixer
	docker-compose run --rm --no-deps $(PHP_FPM) sh -lc './vendor/bin/ecs check src tests --fix'

tests-unit: ## Run unit-tests suite
	docker-compose run --rm --no-deps $(PHP_FPM) sh -lc "php ./vendor/codeception/codeception/app.php -c ./tests/unit/codeception.yml run $(RUN_ARGS)"

tests-integration: ## Run integration-tests suite
	docker-compose run --rm --no-deps $(PHP_FPM) sh -lc "php ./vendor/codeception/codeception/app.php -c ./tests/integration/codeception.yml run $(RUN_ARGS)"
...
phpstan: ## phpstan - PHP Static Analysis Tool
	docker-compose run --rm --no-deps $(PHP_FPM) sh -lc './vendor/bin/phpstan analyse -l 6 -c phpstan.neon src tests'

.PHONY: psalm
psalm: ## psalm is a static analysis tool for finding errors in PHP applications
	docker-compose run --rm --no-deps $(PHP_FPM) sh -lc './vendor/bin/psalm --config=psalm.xml'

.PHONY: phan
phan: ## phan is a static analyzer for PHP that prefers to minimize false-positives.
	docker-compose run --rm --no-deps $(PHP_FPM) sh -lc 'PHAN_DISABLE_XDEBUG_WARN=1 PHAN_ALLOW_XDEBUG=0 php -d zend.enable_gc=0 ./vendor/bin/phan --config-file .phan/config.php --require-config-exists'
